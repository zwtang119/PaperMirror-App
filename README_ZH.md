...
*   **零 Token 保真检测**: 默认只计算数字/缩写保留率与告警，完全本地规则。
*   **可选完整分析**: 需要镜像分数、风格对比、引用建议时，可在 `config.ts` 切换到 `full` 模式。
...
在 [`config.ts`](./config.ts) 配置分析模式：

| 模式 | 行为 |
| --- | --- |
| `fidelityOnly`（默认） | 仅运行本地保真检测（数字/缩写保留率 + 告警），零 Token。 |
| `none` | 完全跳过报告生成，最快返回三档文本。 |
| `full` | 恢复完整的镜像分数、风格对比与引用建议分析。 |
