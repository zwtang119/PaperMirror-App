...
*   **更稳定的模型后备**：在 Gemini 请求失败时，增加可配置的备用模型（如 Claude/DeepSeek）与自动重试，保证长文处理不中断。
*   **可视化质量对比**：提供“润色前后对照”视图与可调节的风格强度滑杆，让用户按需平衡改写力度。
*   **批量与章节队列**：支持多文件/章节排队处理与断点续跑，方便大型论文分段迁移风格。
*   **安全与合规增强**：默认启用本地沙盒解析、离线词表过滤与 Key 配额提示，降低泄露风险并提醒用户合理使用。
